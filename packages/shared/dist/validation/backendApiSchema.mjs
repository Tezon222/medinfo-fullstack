...
const stringWithNumberValidation = (key) => z.transform(String).transform(Number).refine((value) => !Number.isNaN(value), `${key} must be a number`).refine((value) => value > 0, `${key} must be greater than 0`);
...
const diseaseRoutes = defineSchemaRoutes({
	...
	"@get/diseases/all": {
		...
		query: z.object({
			...
			random: z.transform(String).transform((value) => {
